    ...
    <div class="container">
        <h1 class="h4 mb-4">ویرایش مجتمع: {{ $complex->name }}</h1>
        ...
        <form action="{{ route('complexes.update', $complex) }}" method="POST" class="card p-4">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3">
                ...
                <input type="text" id="name" name="name"
                       value="{{ old('name', $complex->name) }}" class="form-control" required>
            </div>

            <div class="mb-3">
                <label for="address" class="form-label">آدرس</label>
                <textarea id="address" name="address" rows="3"
                          class="form-control" required>{{ old('address', $complex->address) }}</textarea>
            </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" id="floors" name="floors" min="1"
                           value="{{ old('floors', $complex->floors) }}" class="form-control" required>
                </div>

                <div class="col-md-6 mb-3">
                    <label for="parking_spaces" class="form-label">جای پارک</label>
                    <input type="number" id="parking_spaces" name="parking_spaces" min="0"
                           value="{{ old('parking_spaces', $complex->parking_spaces) }}" class="form-control" required>
                </div>
            </div>
                ...
                <select id="manager_person_id" name="manager_person_id" class="form-select">
                    ...
                    @foreach ($people as $person)
                        <option value="{{ $person->id }}"
                            {{ old('manager_person_id', $complex->manager_person_id) == $person->id ? 'selected' : '' }}>
                            {{ $person->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="d-flex justify-content-end">
                ...
                <button class="btn btn-success ms-2">ذخیره تغییرات</button>
            </div>
        </form>
    </div>
